...
module Text.XML.Stream.Parse
    ...
    , ignoreAllTreesContent
      -- * Lookahead parsing of tags
    , lookahead
    , lookaheadPredicate
    , lookaheadName
    , lookaheadNoAttr
    , lookaheadIgnoreAttrs
    , lookaheadPredicateIgnoreAttrs
      -- * Attribute parsing
    , AttrParser
    , attr
    , requireAttr
...
dropWhiteSpace :: Monad m => [Event] -> Consumer Event m (Maybe Event, [Event])
dropWhiteSpace leftovers = do
    x <- await
    let leftovers' = maybe id (:) x leftovers

    case isWhitespace <$> x of
        Just True -> dropWhiteSpace leftovers'
...
genericTag :: MonadThrow m
    => (Name -> Maybe a)
    -> (a -> AttrParser b)
    -> (Event -> b -> CI.ConduitM Event o m (Maybe c))
    -> CI.ConduitM Event o m (Maybe c)
genericTag checkName attrParser f = do
    (x, leftovers) <- dropWhiteSpace []
    res <- case x of
        Just evt@(EventBeginElement name as) ->
    ...
    case res of
        ...
        -- final observed event generated by dropWhiteSpace
...
tag :: MonadThrow m
    ...
    -> CI.ConduitM Event o m (Maybe c)
tag checkName attrParser f = genericTag checkName attrParser $ \(EventBeginElement name _) z -> do
    z' <- f z
    (a, _leftovers') <- dropWhiteSpace []
    case a of
...
lookahead
    ...
    -> CI.ConduitM Event o m (Maybe c)
lookahead checkName attrParser f = genericTag checkName attrParser $ \evt z -> do
    leftover evt
    z' <- takeAllTreesContent .| f z
    return $ Just z'
...
lookaheadPredicate
    ...
    -> CI.ConduitM Event o m (Maybe b)
lookaheadPredicate p attrParser = lookahead (guard . p) (const attrParser)
...
lookaheadName
    ...
    -> (a -> CI.ConduitM Event o m b)
    -> CI.ConduitM Event o m (Maybe b)
lookaheadName name = lookaheadPredicate (== name)
...
lookaheadNoAttr
    ...
    -> CI.ConduitM Event o m (Maybe a)
lookaheadNoAttr name f = lookaheadName name (return ()) $ const f
...
lookaheadIgnoreAttrs
    ...
    -> CI.ConduitM Event o m a
    -> CI.ConduitM Event o m (Maybe a)
lookaheadIgnoreAttrs name f = lookaheadName name ignoreAttrs $ const f
...
lookaheadPredicateIgnoreAttrs
    ...
    -> CI.ConduitM Event o m a
    -> CI.ConduitM Event o m (Maybe a)
lookaheadPredicateIgnoreAttrs namePred f = lookaheadPredicate namePred ignoreAttrs $ const f
